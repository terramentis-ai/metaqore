...
class ComplianceAuditor:
    ...
    async def run_compliance_check(self, framework: str, scope: Dict[str, Any], evidence_types: List[str]) -> Dict[str, Any]:
        """Run a compliance check against a specific framework."""
        if framework not in self._frameworks:
            raise ValueError(f"Unsupported compliance framework: {framework}")

        framework_impl = self._frameworks[framework]
        return await framework_impl.check_compliance(scope, evidence_types, self)

    async def collect_evidence(self, evidence_type: str, data: Dict[str, Any], source: str) -> str:
        """Collect and store evidence for compliance purposes."""
        import uuid
...
class ComplianceFramework:
    ...
    async def check_compliance(self, scope: Dict[str, Any], evidence_types: List[str], auditor: 'ComplianceAuditor') -> Dict[str, Any]:
        """Check compliance against this framework."""
...
class SOC2Framework(ComplianceFramework):
    ...
    async def check_compliance(self, scope: Dict[str, Any], evidence_types: List[str], auditor: 'ComplianceAuditor') -> Dict[str, Any]:
        ...
        # Check for audit logs (security control)
        if "audit_logs" in evidence_types:
            audit_trail = auditor.get_audit_trail()
            if len(audit_trail) == 0:
                results["violations"].append("No audit logs found - SOC2 requires audit logging")
                results["status"] = "non_compliant"
            else:
                results["controls"]["audit_logging"] = "compliant"

        # Check for data encryption (confidentiality control)
        if "data_protection" in evidence_types:
            # This would check for encryption evidence
            results["controls"]["data_encryption"] = "unknown"
...
class GDPRFramework(ComplianceFramework):
    ...
    async def check_compliance(self, scope: Dict[str, Any], evidence_types: List[str], auditor: 'ComplianceAuditor') -> Dict[str, Any]:
        ...
        # GDPR focuses on data protection, consent, data minimization, etc.
        if "data_processing" in evidence_types:
            audit_trail = auditor.get_audit_trail()
            # Check for data processing activities
            data_processing_events = [e for e in audit_trail if "data" in str(e.get("payload", {})).lower()]
            if len(data_processing_events) > 0:
                results["controls"]["data_processing_inventory"] = "compliant"
            else:
                results["violations"].append("No data processing activities documented")
                results["status"] = "non_compliant"
...
class HIPAAFramework(ComplianceFramework):
    ...
    async def check_compliance(self, scope: Dict[str, Any], evidence_types: List[str], auditor: 'ComplianceAuditor') -> Dict[str, Any]:
        ...
        # HIPAA focuses on PHI protection, security safeguards, etc.
        if "health_data" in evidence_types:
            audit_trail = auditor.get_audit_trail()
            # Check for health data handling
            health_events = [e for e in audit_trail if any(term in str(e.get("payload", {})).lower()
                                                         for term in ["health", "medical", "phi", "patient"])]
            if len(health_events) > 0:
                results["controls"]["phi_protection"] = "compliant"
            else:
                results["violations"].append("No PHI handling activities documented")
                results["status"] = "non_compliant"
