"""PostgreSQL-backed implementation of the storage backend interface."""
...
class PostgreSQLBackend(StorageBackend):
    """SQLAlchemy-powered PostgreSQL backend."""

    ARTIFACT_MODEL_REGISTRY: Dict[str, Type[Artifact]] = {
        "specialist_model": SpecialistModel,
    }

    def __init__(self, dsn: str = "postgresql://user:password@localhost:5432/metaqore") -> None:
    ...
    def list_projects(self) -> List[Project]:
        with Session(self.engine) as session:
            rows = session.execute(select(ProjectTable)).scalars().all()
            projects = []
            for row in rows:
                project = Project.model_validate(row.payload)
    ...
    def get_artifact(self, artifact_id: str) -> Optional[Artifact]:
        with Session(self.engine) as session:
            ...
            if not row:
                return None
            model_cls = self.ARTIFACT_MODEL_REGISTRY.get(row.artifact_type, Artifact)
            artifact = model_cls.model_validate(row.payload)
            return artifact

    def list_artifacts(self, project_id: str) -> List[Artifact]:
        with Session(self.engine) as session:
            stmt = select(ArtifactTable).where(ArtifactTable.project_id == project_id)
    ...
    def list_tasks(self, project_id: str) -> List[Task]:
        with Session(self.engine) as session:
            stmt = select(TaskTable).where(TaskTable.project_id == project_id)
            rows = session.execute(stmt).scalars().all()
            tasks = []
            for row in rows:
                task = Task.model_validate(row.payload)
    ...
    def save_conflicts(self, conflicts: Iterable[Conflict]) -> None:
        with Session(self.engine) as session:
            for conflict in conflicts:
                ...
                existing = session.get(ConflictTable, conflict.id)
                if existing:
                    existing.payload = payload
                    existing.resolved = conflict.resolved
                    existing.resolved_at = conflict.resolved_at
                else:
                    session.add(
                        ConflictTable(
                            id=conflict.id,
